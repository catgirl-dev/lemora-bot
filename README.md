...
Также есть функционал для автоматического поздравления участников чата с Днём рождения.
...
- Добавление/удаление/изменение Дней рождения участников чата.
...
- `/add_birthday` — добавляет День рождения по ID пользователя.
- `/change_birthday` — изменяет День рождения по ID пользователя.
- `/delete_birthday` — удаляет День рождения по ID пользователя.
- `/get_all_birthdays`— возвращает Дни рождения участников чата.
...
- **Команда `/change_ban_time <время в секундах>`** устанавливает время, на которое блокируется пользователь в случае непрохождения капчи. Пример:
  ...
  /change_ban_time 120
  ```
- **Команда `/add_birthday <ID пользователя> <MM-DD>`** добавляет День рождения пользователя в БД. Пример:
  ```
  /add_birthday 12345 12-01
  ```
- **Команда `/change_birthday <ID пользователя> <MM-DD>`** изменяет День рождения пользователя в БД. Пример:
  ```
  /change_birthday 12345 12-01
  ```
- **Команда `/delete_birthday <ID пользователя>`** удаляет День рождения пользователя из БД. Пример:
  ```
  /delete_birthday 12345
  ```
- **Команда `/get_all_birthdays`** возвращает все Дни рождения участников чата. Пример:
  ```
  /get_all_birthdays
  ```
